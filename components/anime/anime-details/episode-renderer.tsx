...
const AnimatedEpisodes: React.FC<PaginatedEpisodesProps> = ({ episodes, type }) => {
    ...
    const filteredEpisodes = hideFillers ? episodes.filter(episode => !episode.isFiller) : episodes;
    ...
    return (
        <div className='flex gap-4  flex-col'>
            {filteredEpisodes.length > EPISODES_PER_PAGE && (
                <div className="mt-4">
                    <Select onValueChange={(value) => handleSelectPage(parseInt(value))} value={String(currentPage)}>
                        ...
                        <SelectContent className='p-1'>
                            {Array.from({ length: totalPages }).map((_, index) => {
                                ...
                                const end = Math.min((index + 1) * EPISODES_PER_PAGE, filteredEpisodes.length);
                                return (
                ...
                {currentEpisodes.map((episode) => (
                    ...
                    >
                        {!isMovie ? <>
                            ...
                            <p className="line-clamp-2 text-sm md:text-base font-bold">{episode.title}</p>
                            {episode.isFiller && (
